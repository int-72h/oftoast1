...
parser.add_argument("-k", default=None, help="keyfile")
...
latest_revision = fetch_latest_revision(url,key)
installed_revision = get_installed_revision(game_path)

print(installed_revision, "->", latest_revision, file=stderr)

revisions = fetch_revisions(url, installed_revision, latest_revision,key)
...
todl.close()
fp = Popen('aria2c -i todl.txt -d / -x 100 -j 100 -m 10 -V -U murse/0.0.2', shell=True,stdin=PIPE, stdout=PIPE, universal_newlines=True)
